...
alpha(1) = 0;
grad = 1./m * X' * (sigmoid(X * theta) - y) + lambda * (theta .* alpha)/ m;

%J = (-1./m * y*log(sigmoid(X * theta)) + (1 -y)*log(1-X * theta)) + lambda/(2*m) * theta^2
%grad = (1/m * X * sigmoid(X * theta)) + theta * (lambda/m)
